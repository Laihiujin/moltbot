...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - the Windows release lane builds NSIS and MSI installers, signs and verifies
    them when `WINDOWS_PFX_BASE64` is configured, skips signing when the secret
    is absent, runs separate smoke install jobs, and uploads the installers plus
    a SHA256 manifest to the matching GitHub release
